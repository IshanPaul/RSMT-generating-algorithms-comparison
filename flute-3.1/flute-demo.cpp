...
void solve(int u){
    ...
    cout<<"Enter np. of terminals:";
    cin>>n;

    /* Generate NUM_INSTANCES random instances with NUM_TERMS terminals */
    srand48 (1);
    /* Generate random points with coordinates in range 0..9999 */
    cout<<"Enter to terminal coordinates one by one:-\n";
    f(i,0,n)cin>>x[i]>>y[i];
    /* Compute Steiner tree and print length */
    readLUT();
    Start;
    flutetree = flute(n,x,y,ACCURACY);
    cout<<"Instance "<<u<<" has FLUTE wirelength ="<<flutetree.length<<ln;
    cout<<"FLUTE wirelength (without RSMT construction) ="<<flute_wl(n,x,y,ACCURACY)<<ln;
    End(milliseconds);
}
int main(){
    // #ifndef ONLINE_JUDGE  
