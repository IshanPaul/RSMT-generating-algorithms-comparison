...
echo -e "\nExecution completed successfully."This project provides a **Bash-based simulation framework** for generating and testing Steiner Tree algorithms with multiple backends:
- **Flute** (via `flute-3.1`)
- **GeoSteiner** (via `geosteiner-5.3`)
- **Cockayne (Ishan’s implementation)** (via `Steiner Tree/Code/RSMT.cpp`)

The script automates:
1. Input file generation with random terminals (or reading an existing file).
2. Running the selected algorithm(s).
3. Collecting outputs and logs for analysis.
